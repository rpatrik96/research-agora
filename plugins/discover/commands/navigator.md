...
for repo in d['repos']:
    for s in repo['skills']:
        ...
        print(f\"{s['name']}\t{s['plugin']}\t{tools}\t{' '.join(s['description'].split())[:110]}\")
"
```

Each row gives you the skill, the plugin it installs from, the tools it
actually invokes, and its trigger phrases. Match the user's words against the
descriptions, and prefer a skill that invokes a tool over one that does not
when both would answer the request — that is the marketplace's own rule.

A hardcoded table here is how this skill previously came to recommend eight
skills that no longer existed. The registry is regenerated by
`scripts/generate-registry.py` on every change; a table in this file is not.

**If `registry/index.json` is not readable**, say so and route from the plugin
names alone (`discover`, `write`, `verify`, `toolkit`) rather than guessing at
skill names.
